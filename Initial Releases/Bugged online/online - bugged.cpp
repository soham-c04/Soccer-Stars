...
int moves=0;	 	// Counts the number of moves played. Ensures that the first shot is not a goal
...
void Time(bool send = false){ // time function which updates the time displayed in seconds in the screen.
	time(&tf);
	if(ti!=tf){
		ti=tf;
		Timer++;
...
class Ball{
	private:
		const double loss=8.0; 	  // Reduction in speed of ball /sec
	public:
...
class Goal{
	public:
		...
		int goal_1=0,goal_2=0;  // goal counter for first and second player respectively
  		...
  		void goaling(int who){ // This is initiated whenever a goal happens. Here who defines whoever accepted the goal
			turn=who;
			ball.speed=3.0;
			if(who==1){
				if(moves>1) goal_2++; // Ensures no goal on first strike
				while(ball.x>X1-2*width/3 && (ball.y>=(Y2+Y1)/2-height+ball.radius) && (ball.y<=(Y2+Y1)/2+height-ball.radius)){
					/*
				...
				if(moves>1) goal_1++;
				while(ball.x<X2+2*width/3 && (ball.y>=(Y2+Y1)/2-height+ball.radius) && (ball.y<=(Y2+Y1)/2+height-ball.radius)){
					_print();
					ball.move(true);
					striker1.move();
					striker2.move();
			...
			//occured
			if(moves>1) readimagefile("resources/goal.jpg",(X1+X2)/2-150*Xratio,(Y1+Y2)/2-75*Yratio,(X1+X2)/2+150*Xratio,(Y1+Y2)/2+75*Yratio);
			else{
				setcolor(13);
				settextstyle(0,0,5);
...
/*
Check_Collision() boolean type function was defined instead of just a normal collisions because in normal collisions function
...
//		settextstyle(3,HORIZ_DIR,7);
//		outtextxy((X1+X2)/2-320*Xratio, (Y1+Y2)/2 - 100*Yratio, waiting);
//		setvisualpage(page);
//		setactivepage(1-page);
//		page = 1-page;
//		settextstyle(4,HORIZ_DIR,3*Xratio);
...
int main_(){
	...
	if(Status == ONLINE){
		...
		cin >> port_no;
//		server_details(server_ip, port_no);
		while(connect_to_server(server_ip,port_no)) ;
	//		server_details(server_ip, port_no);

		cout << endl << "Connected to server!" << endl;
	}
	srand(time(0)); // generates a random number to determine who will start the game (for OFFLINE)
	start();
